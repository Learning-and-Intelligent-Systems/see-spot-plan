...
def create_synchronized_command(arm_joint_traj, gripper_percentage=None, gripper_force=None):
    ...
    try:
        while timestep_index < len(timesteps):
            ...
            if gripper_open_percentage > 1.0:
                gripper_open_percentage = min(max(gripper_open_percentage / 100.0, 0.0), 1.0)
            
            # Adjust gripper openness based on force detection - for stronger grip
            if gripper_force and "magnitude" in gripper_force and gripper_force["magnitude"] > 10.0:
                ...
                print(f"High force detected ({gripper_force['magnitude']:.2f}N) - reducing gripper opening from "
            ...
            if last_gripper_percentage is None or abs(gripper_open_percentage - last_gripper_percentage) > 0.02:
                ...
                print(f"Including gripper position {gripper_open_percentage:.2f} in command")
            ...
            combined_command = create_synchronized_command(arm_joint_traj, gripper_to_send, gripper_force)
